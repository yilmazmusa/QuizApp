...
function optionSelected(option) {
  clearInterval(
    counter
  ); /* BİR SEÇENEK SÜRE BİTMEDEN SEÇİLDİĞİNDE ONA GÖRE İŞLEM YAPILACAK */
  clearInterval(
    counterLine
  ); /* BİR SEÇENEK SEÇİLDİĞİNDE YATAY AKAN TİME_LİNE DURMASI İÇİN */
...
function startTimer(time) {
  counter = setInterval(
    timer,
    1000
  ); /*setInterval metodu timer metodunu 1 saniyede bir çağıracak */
  ...
  function timer() {
    ...
    if (time < 0) {
      clearInterval(
        counter
      ); /* Temizlenece olan Interval'in referansını verdik.*/
